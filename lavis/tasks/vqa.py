...
class VQATask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, **kwargs):
        ...
        save_data = {"VLAR":[]}
        for i, data in enumerate(val_result):
            puz = {'Id': data['question_id'], 'Answer': data['answer']}
            save_data["VLAR"].append(puz)
        json.dump(save_data, open("submission/submission.json",'w'))

        print("submission file: save to submission/submission.json")
        print("====================================================================================================")

        # with open(os.path.join("/submission/", 'submission.json'), 'w') as pred_json:
        ...
        # for r in val_result:
        #     if r['info'] not in res:
        #         res[r['info']] = [0,0]
        #     if r['answer'] == r['label']:
        #         res[r['info']][0]+=1
        #     res[r['info']][1]+=1
        # for k,v in res.items():
        #     print(f"acc: {k} = {v[0]/v[1]}")

        # preds = np.array([x['answer'] for x in val_result])
        # labels = np.array([x['label'] for x in val_result])

        # metrics = np.sum(preds==labels)/len(preds)
        # print(f"avg_acc: {metrics}")
